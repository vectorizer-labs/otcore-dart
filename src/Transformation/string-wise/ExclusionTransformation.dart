...
class ExclusionTransformation
{
  ...
  {
    //Restores the Absolute index of Oa if
    //1) its a relatively addressed Operation
    //2) Ob is the Operation its relatively addressed to
    //TODO: BROKEN?
    //if(dc.Check_RA(Oa)) return OperationResult.from_single(Oa); //= dc.Restore_RA(Oa, Ob); 


    if(Oa.is_insert && Ob.is_insert) return ET_II(Oa, Ob, dc);
    else if(Oa.is_insert && !Ob.is_insert) return ET_ID(Oa,Ob, dc);
    else if(!Oa.is_insert && Ob.is_insert) return ET_DI(Oa, Ob);
    else /*if(!Oa.is_insert && !Ob.is_insert)*/ return ET_DD(Oa, Ob, dc);
  }

  static OperationResult ET_II(Operation Oa, Operation Ob, DocState dc)
  {
    ...
    {
      //Save_RA
      Operation OaPrime = Operation.from(Oa.set_index(Oa.index - Ob.index));
      //dc.Save_RA(OaPrime, Ob);

      return OperationResult.from_single(OaPrime); 
    }
  }

  static OperationResult ET_ID(Operation Oa, Operation Ob, DocState dc)
  {
    //Check LI
    //Oa = dc.Check_LI(Oa);
  ...
  static OperationResult ET_DD(Operation Oa, Operation Ob, DocState dc)
  {
    //Check_LI
    //Oa = dc.Check_LI(Oa);
